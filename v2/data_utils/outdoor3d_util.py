...
def collect_point_label(anno_path, out_filename, file_format='txt'):
    ...
    """
    # points_list = []
    for f in set(glob.glob(os.path.join(anno_path, '*.txt'))):
        ...
        # print('pre-open')
        points = np.loadtxt(f, delimiter=',', usecols=(0,1,2,3,4,5)).astype(np.float32)
        labels = np.loadtxt(f, delimiter=',', usecols=(6)).astype(np.int64)
        # print('pre-change')
        labels[labels == 6] = 0
        labels[labels == 26] = 2
        points = points[labels != 9]
        labels = labels[labels != 9]
        labels = labels.reshape(labels.shape[0],1)
        data_label = np.concatenate([points, labels], axis=1)
        xyz_min = np.amin(data_label, axis=0)[0:3]
        data_label[:, 0:3] -= xyz_min
        # print('pre-save')
        if file_format=='txt':
            fout = open(out_filename + '_' + f.split('.')[0], 'w')
